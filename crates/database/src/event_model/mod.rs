...
pub struct NftClaimEvent {
    ...
    /// 推荐人地址（可选）
    #[serde(with = "option_pubkey_string")]
...
mod option_pubkey_string {
    use serde::{Deserialize, Deserializer, Serialize, Serializer};
    use solana_sdk::pubkey::Pubkey;

    pub fn serialize<S>(opt_pubkey: &Option<Pubkey>, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        match opt_pubkey {
            Some(pubkey) => Some(pubkey.to_string()).serialize(serializer),
