...
pub fn swap_base_output(ctx: Context<Swap>, max_amount_in: u64, amount_out_received: u64) -> Result<()> {
    ...
    let (input_transfer_amount, input_transfer_fee) = {
        ...
        let transfer_fee = get_transfer_inverse_fee(&ctx.accounts.input_token_mint.to_account_info(), input_amount)?;
    ...
    let (output_transfer_amount, output_transfer_fee) = (amount_out_with_transfer_fee, out_transfer_fee);
