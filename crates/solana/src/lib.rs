...
pub use raydium::{RaydiumPoolInfo, RaydiumSwap, SwapEstimateResult};
...
mod tests {
    ...
    async fn test_raydium_swap_calculation() {
        ...
        let client = SolanaClient::new(&config).expect("Failed to create client");

        let raydium_swap = RaydiumSwap::new(client, &config).expect("Failed to create RaydiumSwap");
