...
impl RaydiumSwap {
    ...
    async fn calculate_clmm_output(
        ...
        // 直接使用client模块的精确计算
        match self.try_get_pool_info_with_client(
            ...
            Err(e) => {
                ...
                match self.load_swap_accounts(pool_pubkey, zero_for_one).await {
                    Ok(accounts_data) => {
                        match self.call_client_precise_calculation(
                            amount_in,
                            zero_for_one,
                            &accounts_data,
                        ).await {
    ...
    async fn calculate_amm_v4_output(
        ...
        match self.get_amm_v4_pool_reserves(pool_pubkey, from_mint, to_mint).await {
            Ok((reserve_in, reserve_out, fee_rate)) => {
                ...
                // 使用浮点数进行精确计算
                let output_f64 = (amount_in_after_fee_f64 * reserve_out as f64) / 
                                 (reserve_in as f64 + amount_in_after_fee_f64);
        ...
        for (start, end) in offsets {
            if end <= pool_account.data.len() {
                let value = u64::from_le_bytes(
                    pool_account.data[start..end].try_into().unwrap_or([0; 8])
                );
                // 过滤掉明显无效的值
    ...
    async fn get_clmm_pool_vault_balances(
        &self,
        pool_pubkey: &Pubkey,
        from_mint: &str,
        to_mint: &str,
        ...
        // 1. 先获取池子状态
        let pool_account = rpc_client.get_account(pool_pubkey)?;
        let pool_state = client::deserialize_anchor_account::<raydium_amm_v3::states::PoolState>(
        ...
        let amm_config_account = match rpc_client.get_account(&pool_state.amm_config) {
            ...
            Err(e) => {
                ...
                return self.determine_vault_direction(
                    &pool_state, 
                    from_mint, 
                    to_mint, 
                    vault_0_balance, 
        ...
        let amm_config = client::deserialize_anchor_account::<raydium_amm_v3::states::AmmConfig>(
            &solana_sdk::account::Account {
        ...
        // 4. 确定交换方向
        self.determine_vault_direction(&pool_state, from_mint, to_mint, vault_0_balance, vault_1_balance, fee_rate)
    }
    
    /// 确定vault交换方向
    fn determine_vault_direction(
        &self,
        ...
        let (reserve_in, reserve_out) = if pool_state.token_mint_0 == from_mint_pubkey && pool_state.token_mint_1 == to_mint_pubkey {
            ...
            return Err(anyhow::anyhow!(
                ...
                from_mint, to_mint, pool_state.token_mint_0, pool_state.token_mint_1
            ));
        };
    ...
    async fn get_amm_v4_pool_info(
        ...
        match self.try_get_pool_info_with_client(pool_pubkey.to_string().as_str(), from_mint, to_mint, 1000000).await {
            Ok(_) => {
        ...
        for (start, end) in vault_offsets {
            if end <= pool_account.data.len() {
                ...
                if amount > 0 && amount < u64::MAX / 2 { // 过滤掉异常大的值
    ...
    async fn try_get_pool_info_with_client(
        ...
        let bitmap_account = rpc_client.get_account(&tickarray_bitmap_pubkey)
        ...
        // 加载必要的tick arrays
        let mut tick_arrays = self.load_tick_arrays_for_calculation(
            &pool_pubkey,
            &pool_state,
            &tickarray_bitmap,
            zero_for_one,
    ...
    async fn call_client_precise_calculation(
        ...
        info!("  ⚙️ AMM配置: 手续费={}, tick_spacing={}", 
              trade_fee_rate, tick_spacing);
    ...
    /// 创建默认的tick数组数据
    fn create_default_tick_arrays(&self, count: usize) -> Vec<Vec<u8>> {
    ...
    async fn get_swap_accounts(
        ...
        // 获取需要的tick arrays
        let tick_arrays = self.get_required_tick_arrays(
            pool_pubkey,
            pool_state,
            zero_for_one,
        ).await?;
    ...
    async fn send_swap_transaction(&self, instruction: Instruction) -> Result<String> {
        ...
        let recent_blockhash = self.client.get_rpc_client()
        ...
        let signature = self.client.get_rpc_client()
    ...
    /// 创建关联代币账户（如果不存在）
    pub async fn ensure_associated_token_accounts(
        &self,
        mint_addresses: &[&str],
    ) -> Result<Vec<String>> {
        info!("🔧 确保关联代币账户存在");
    ...
    pub async fn smart_swap(
        ...
        // 4. 执行交换
        let signature = self.execute_clmm_swap(
            input_mint,
            output_mint,
            pool_address,
            input_amount,
    ...
    /// SOL到USDC的交换（向后兼容方法）
    pub async fn swap_sol_to_usdc_with_pool(&self, pool_address: &str, amount_in_lamports: u64, minimum_amount_out: u64) -> Result<String> {
        info!("🔄 SOL到USDC交换");
        ...
        self.smart_swap(
            ...
            Some(50), // 0.5% 滑点
            Some(500), // 5% 最大价格影响
        ).await.map(|result| result.signature)
    }

    /// USDC到SOL的交换（向后兼容方法）
    pub async fn swap_usdc_to_sol_with_pool(&self, pool_address: &str, amount_in_usdc: u64, minimum_amount_out: u64) -> Result<String> {
        info!("🔄 USDC到SOL交换");
    ...
    pub async fn calculate_swap_output_direct(&self, pool_address: &str, from_mint: &str, to_mint: &str, amount_in: u64) -> Result<u64> {
        ...
        let tick_bitmap_extension = rpc_client.get_account(&tick_bitmap_extension_pubkey)
        ...
        // 获取当前 tick array
        let (is_pool_current_tick_array, current_tick_array_start_index) = pool_state
        ...
        let tick_array_account = rpc_client.get_account(&tick_array_pubkey)
