...
impl TickArrayState {
    ...
    pub fn get_or_create_tick_array<'info>(
        ...
        let tick_array_state = if tick_array_account_info.owner == &system_program::ID {
            ...
            let tick_array_state_loader =
                AccountLoad::<TickArrayState>::try_from_unchecked(&crate::id(), &tick_array_account_info)?;
            {
                let mut tick_array_account = tick_array_state_loader.load_init()?;
                tick_array_account.initialize(tick_array_start_index, tick_spacing, pool_state_loader.key())?;
            }
    ...
    pub fn next_initialized_tick(
        ...
        // 先复制字段值到本地变量避免 packed field 问题
        let start_tick_index = self.start_tick_index;

        println!("next_initialized_tick 调用:");
        println!("  current_tick_index: {}", current_tick_index);
        println!("  tick_spacing: {}", tick_spacing);
        println!("  zero_for_one: {}", zero_for_one);
        println!("  array start_tick_index: {}", start_tick_index);

        let current_tick_array_start_index = TickArrayState::get_array_start_index(current_tick_index, tick_spacing);
        println!("  calculated array start: {}", current_tick_array_start_index);

        if current_tick_array_start_index != self.start_tick_index {
            println!("  返回 None: array start 不匹配");
            return Ok(None);
        }

        let mut offset_in_array = (current_tick_index - self.start_tick_index) / i32::from(tick_spacing);
        println!("  初始 offset_in_array: {}", offset_in_array);

        if zero_for_one {
            while offset_in_array >= 0 {
                println!(
                    "  检查 offset {}: tick_index = {}",
                    offset_in_array,
                    self.start_tick_index + offset_in_array * i32::from(tick_spacing)
                );

                if self.ticks[offset_in_array as usize].is_initialized() {
                    println!("  找到初始化的 tick at offset {}", offset_in_array);
                    return Ok(self.ticks.get_mut(offset_in_array as usize));
                }
                offset_in_array = offset_in_array - 1;
            }
            println!("  zero_for_one: 未找到初始化的 tick");
        } else {
            offset_in_array = offset_in_array + 1;
            while offset_in_array < TICK_ARRAY_SIZE {
                println!(
            ...
            println!("  one_for_zero: 未找到初始化的 tick");
        }
        Ok(None)
    }
    ...
    pub fn check_is_valid_start_index(tick_index: i32, tick_spacing: u16) -> bool {
        if TickState::check_is_out_of_boundary(tick_index) {
            ...
            let min_start_index = TickArrayState::get_array_start_index(tick_math::MIN_TICK, tick_spacing);
...
// TickState
// tick: 每个Tick的索引值
// liquidity_net: 当价格从左到右穿越此Tick时的净流动性变化(可为负)
// liquidity_gross: 引用此Tick的总流动性(始终为正)
// fee_growth_outside_0_x64: token0 在此tick外侧的手续费增长
// fee_growth_outside_1_x64: token1 在此tick外侧的手续费增长
// reward_growths_outside_x64: 奖励增长数组
#[zero_copy(unsafe)]
#[repr(C, packed)]
...
impl TickState {
    ...
    // 初始化对应Tick: Tick值在有效范围内，且值必须是tick_spacing的整数倍
    ...
    pub fn update(
        &mut self,
        tick_current: i32,
        liquidity_delta: i128, // 流动性变化量(可正可负)
        fee_growth_global_0_x64: u128,
        fee_growth_global_1_x64: u128,
        upper: bool, // 是否为Position的上边界
    ...
    // Tick穿越：当价格穿越此tick时调用，更新fee和reward的"外侧"记录
...
pub fn check_ticks_order(tick_lower_index: i32, tick_upper_index: i32) -> Result<()> {
    require!(tick_lower_index < tick_upper_index, ErrorCode::TickInvaildOrder);
    Ok(())
}
...
pub mod tick_array_test {
    ...
    mod tick_array_test {
        ...
        fn get_array_start_index_test() {
            ...
            assert_eq!(TickArrayState::get_array_start_index(30720, 1), 30720);
            assert_eq!(TickArrayState::get_array_start_index(tick_math::MIN_TICK, 1), -443640);
            assert_eq!(TickArrayState::get_array_start_index(tick_math::MAX_TICK, 1), 443580);
            assert_eq!(TickArrayState::get_array_start_index(tick_math::MAX_TICK, 60), 442800);
            assert_eq!(TickArrayState::get_array_start_index(tick_math::MIN_TICK, 60), -446400);
        }
    ...
    mod get_reward_growths_inside_test {
        ...
        fn uninitialized_reward_index_test() {
            ...
            let reward_inside = get_reward_growths_inside(tick_lower, tick_upper, tick_current, reward_infos);
