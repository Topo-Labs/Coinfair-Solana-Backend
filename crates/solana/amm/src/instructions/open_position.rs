...
pub fn open_position<'a, 'b, 'c: 'info, 'info>(
    ...
    let mut liquidity = liquidity;
    {
        ...
        check_tick_array_start_index(tick_array_lower_start_index, tick_lower_index, pool_state.tick_spacing)?;
        check_tick_array_start_index(tick_array_upper_start_index, tick_upper_index, pool_state.tick_spacing)?;
...
pub fn add_liquidity<'b, 'c: 'info, 'info>(
    ...
    if flip_tick_lower {
        ...
        if before_init_tick_count == 0 {
            pool_state.flip_tick_array_bit(tick_array_bitmap_extension, tick_array_lower.start_tick_index)?;
        }
    }
    if flip_tick_upper {
        ...
        if before_init_tick_count == 0 {
            pool_state.flip_tick_array_bit(tick_array_bitmap_extension, tick_array_upper.start_tick_index)?;
        }
    }
    require!(
    ...
    Ok((amount_0, amount_1, amount_0_transfer_fee, amount_1_transfer_fee))
}
...
fn get_metadata_data(_personal_position_id: Pubkey) -> (String, String, String) {
    return (
        String::from("Coinfair Concentrated Liquidity"),
        String::from("CCL"),
        format!("https://ipfs.io/ipfs/bafkreihntyvfznhnx5l25na36awtxpn4ye3o4ttgdietf2owlt6qoe6yla"),
        // format!(
