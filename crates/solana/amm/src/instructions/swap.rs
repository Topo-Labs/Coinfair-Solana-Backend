...
pub struct SwapSingle<'info> {
    ...
    pub upper_token_account: Option<Account<'info, TokenAccount>>,
    ...
    #[account(
        ...
        bump,
        seeds::program = referral.key(),
        constraint = payer_referral.upper.is_none() || true
    )]
    ...
    pub upper_upper_token_account: Option<Account<'info, TokenAccount>>,
...
pub struct SwapAccounts<'b, 'info> {
    ...
    // // 新增分佣字段
    // /// 输入 token 的 Mint（用于校验 upper ATA）
    // pub input_mint: Box<InterfaceAccount<'info, Mint>>,

    // /// payer 的推荐关系
    // pub payer_referral: Account<'info, ReferralAccount>,

    /// optional 上级（在swap_internal中只用于判断protocol_fee该扣除多少）
    //pub upper: Option<UncheckedAccount<'info>>,

    /// optional 上级奖励账户（input mint 的 ATA）
    pub upper_token_account: Option<Account<'info, TokenAccount>>,

    // /// optional 上上级 referral
    // pub upper_referral: Option<Account<'info, ReferralAccount>>,

    // /// optional 上上级钱包
    // pub upper_upper: Option<UncheckedAccount<'info>>,

    // /// optional 上上级奖励账户（input mint 的 ATA）
    // pub upper_upper_token_account: Option<Account<'info, TokenAccount>>,
...
pub fn exact_internal<'b, 'c: 'info, 'info>(
...

    {
        ...
        (amount_0, amount_1, _) = swap_internal(
            &ctx.amm_config,
            pool_state,
            tick_array_states,
            ...
            // 新增分佣参数
            // &ctx.pool_state.to_account_info(),
            // &ctx.input_vault.to_account_info(),
...
pub fn swap<'a, 'b, 'c: 'info, 'info>(
    ...
    let amount = exact_internal(
        &mut SwapAccounts {
            ...
            // referral_program: ctx.accounts.referral.clone(),            
        },
