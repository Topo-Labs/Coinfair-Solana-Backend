...
impl<'a> AccountLoader<'a> {
    ...
    /// 批量加载账户
    pub async fn load_multiple_accounts(&self, pubkeys: &[Pubkey]) -> Result<Vec<Option<solana_sdk::account::Account>>> {
    ...
    /// 反序列化anchor账户
    pub fn deserialize_anchor_account<T: anchor_lang::AccountDeserialize>(&self, account: &solana_sdk::account::Account) -> Result<T> {
        let mut data: &[u8] = &account.data;
        T::try_deserialize(&mut data).map_err(Into::into)
    }

    /// 批量加载和反序列化核心交换账户
    pub async fn load_swap_core_accounts(&self, pool_pubkey: &Pubkey, input_mint: &Pubkey, output_mint: &Pubkey) -> Result<SwapCoreAccounts> {
        ...
        Ok(SwapCoreAccounts {
            ...
            mint0_state: spl_token_2022::extension::StateWithExtensions::<spl_token_2022::state::Mint>::unpack(&mint0_account.data)?.base,
            mint1_state: spl_token_2022::extension::StateWithExtensions::<spl_token_2022::state::Mint>::unpack(&mint1_account.data)?.base,
            mint0,
            mint1,
