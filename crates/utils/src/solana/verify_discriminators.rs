...
mod tests {
    ...
    fn test_discriminator_unification_verification() {
        ...
        println!(
            "手动计算 open_position_with_token22_nft: {:?}",
            &manual_open_position[..8]
        );
        ...
        let increase_liquidity_match =
            clmm_instruction::IncreaseLiquidityV2::DISCRIMINATOR == &manual_increase_liquidity[..8];
        println!(
            "IncreaseLiquidityV2 匹配: {}",
            if increase_liquidity_match { "✅" } else { "❌" }
        );

        let decrease_liquidity_match =
            clmm_instruction::DecreaseLiquidityV2::DISCRIMINATOR == &manual_decrease_liquidity[..8];
        println!(
            "DecreaseLiquidityV2 匹配: {}",
            if decrease_liquidity_match { "✅" } else { "❌" }
        );
