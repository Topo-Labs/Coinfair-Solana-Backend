...
mod tests {
    ...
    fn test_discriminator_consistency_comprehensive() {
        ...
        println!("✅ OpenPositionV2 一致性: {}", if open_position_match { "PASS" } else { "FAIL" });
        ...
        println!("✅ IncreaseLiquidityV2 一致性: {}", if increase_match { "PASS" } else { "FAIL" });
        if !increase_match { all_tests_passed = false; }
        ...
        println!("✅ DecreaseLiquidityV2 一致性: {}", if decrease_match { "PASS" } else { "FAIL" });
        if !decrease_match { all_tests_passed = false; }
        ...
        if !close_match { all_tests_passed = false; }
        ...
        println!("✅ CP-Swap Initialize 一致性: {}", if cp_init_match { "PASS" } else { "FAIL" });
        if !cp_init_match { all_tests_passed = false; }
