...
mod solana_permission_service_tests {
    ...
    async fn test_fine_grained_api_permission_configuration() {
        ...
        // 更新API配置
        service
            .update_api_config("/api/v1/solana/custom".to_string(), custom_config)
            .await
            .unwrap();
        ...
        let result = service
            .check_api_permission("/api/v1/solana/custom", &SolanaApiAction::Read, &basic_user)
            .await;
        assert!(result.is_err());
        ...
        let result = service
            .check_api_permission("/api/v1/solana/custom", &SolanaApiAction::Read, &premium_user)
            .await;
        assert!(result.is_ok());

        let result = service
            .check_api_permission("/api/v1/solana/custom", &SolanaApiAction::Write, &premium_user)
            .await;
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("用户等级不足"));
        ...
        let result = service
            .check_api_permission("/api/v1/solana/custom", &SolanaApiAction::Read, &vip_user)
            .await;
        assert!(result.is_ok());

        let result = service
            .check_api_permission("/api/v1/solana/custom", &SolanaApiAction::Write, &vip_user)
            .await;
        assert!(result.is_ok());
        ...
        let result = service
            .check_api_permission("/api/v1/solana/custom", &SolanaApiAction::Read, &vip_user_no_perm)
            .await;
        assert!(result.is_ok());

        let result = service
            .check_api_permission("/api/v1/solana/custom", &SolanaApiAction::Write, &vip_user_no_perm)
            .await;
        assert!(result.is_err());
    ...
    async fn test_batch_api_configuration() {
        ...
        // 验证配置生效
        let basic_user = create_test_auth_user(UserTier::Basic, vec![Permission::ReadPool]);
        let premium_user = create_test_auth_user(
            UserTier::Premium,
            vec![Permission::CreatePosition, Permission::ReadPosition],
        );
        let vip_user = create_test_auth_user(
            UserTier::VIP,
            vec![Permission::CreatePosition, Permission::ReadPosition],
        );
        ...
        let result = service
            .check_api_permission("/api/v1/solana/swap", &SolanaApiAction::Read, &basic_user)
            .await;
        assert!(result.is_ok());
        ...
        let result = service
            .check_api_permission("/api/v1/solana/position/open", &SolanaApiAction::Read, &premium_user)
            .await;
        assert!(result.is_ok());

        let result = service
            .check_api_permission("/api/v1/solana/position/open", &SolanaApiAction::Write, &premium_user)
            .await;
        assert!(result.is_ok());

        // Basic用户无权限读取仓位
        let result = service
            .check_api_permission("/api/v1/solana/position/open", &SolanaApiAction::Read, &basic_user)
            .await;
        assert!(result.is_err());

        // 测试池子创建API - 只有管理员可以写入
        let result = service
            .check_api_permission("/api/v1/solana/pool/create/clmm", &SolanaApiAction::Write, &vip_user)
            .await;
        assert!(result.is_err());

        let result = service
            .check_api_permission("/api/v1/solana/pool/create/clmm", &SolanaApiAction::Write, &admin_user)
            .await;
        assert!(result.is_ok());
    }
    ...
    async fn test_emergency_shutdown_override() {
        ...
        // 正常情况下管理员应该可以访问
        let result = service
            .check_api_permission("/api/v1/solana/swap", &SolanaApiAction::Write, &admin_user)
            .await;
        assert!(result.is_ok());
    ...
    async fn test_maintenance_mode_admin_only() {
        ...
        // 普通用户（即使是VIP）应该被拒绝
        let result = service
            .check_api_permission("/api/v1/solana/pools/info/list", &SolanaApiAction::Read, &regular_user)
            .await;
        assert!(result.is_err());
    ...
    async fn test_permission_priority_hierarchy() {
        ...
        service
            .update_api_config("/api/v1/solana/swap".to_string(), disabled_config)
            .await
            .unwrap();
        ...
        service
            .update_api_config("/api/v1/solana/swap".to_string(), enabled_config)
            .await
            .unwrap();
    ...
    async fn test_permission_statistics() {
        ...
        assert_eq!(
            initial_stats.enabled_apis + initial_stats.disabled_apis,
            initial_stats.total_apis
        );
...
mod integration_scenario_tests {
    ...
    async fn test_trading_peak_hour_scenario() {
        ...
        service
            .update_api_config("/api/v1/solana/swap".to_string(), swap_config)
            .await
            .unwrap();
        ...
        for user in [&basic_user, &premium_user, &vip_user] {
            let result = service
                .check_api_permission("/api/v1/solana/swap", &SolanaApiAction::Read, user)
                .await;
            assert!(result.is_ok());
        }
    ...
    async fn test_system_maintenance_scenario() {
        ...
        // 查询功能正常
        let result = service
            .check_api_permission("/api/v1/solana/pools/info/list", &SolanaApiAction::Read, &vip_user)
            .await;
        assert!(result.is_ok());
    ...
    async fn test_feature_gradual_rollout_scenario() {
        ...
        service
            .update_api_config("/api/v1/solana/new-feature".to_string(), new_feature_config)
            .await
            .unwrap();

        // 只有VIP可以访问
        let result = service
            .check_api_permission("/api/v1/solana/new-feature", &SolanaApiAction::Read, &basic_user)
            .await;
        assert!(result.is_err());

        let result = service
            .check_api_permission("/api/v1/solana/new-feature", &SolanaApiAction::Read, &premium_user)
            .await;
        assert!(result.is_err());

        let result = service
            .check_api_permission("/api/v1/solana/new-feature", &SolanaApiAction::Read, &vip_user)
            .await;
        assert!(result.is_ok());
        ...
        service
            .update_api_config("/api/v1/solana/new-feature".to_string(), updated_config)
            .await
            .unwrap();
        ...
        service
            .update_api_config("/api/v1/solana/new-feature".to_string(), final_config)
            .await
            .unwrap();
        ...
        for user in [&basic_user, &premium_user, &vip_user] {
            let result = service
                .check_api_permission("/api/v1/solana/new-feature", &SolanaApiAction::Read, user)
                .await;
            assert!(result.is_ok());

            let result = service
                .check_api_permission("/api/v1/solana/new-feature", &SolanaApiAction::Write, user)
                .await;
