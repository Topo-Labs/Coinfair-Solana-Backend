...
use crate::dtos::solana_dto::{
    ExtendedMintInfo, NewPoolListResponse, PoolInfo, PoolListData, PoolConfigInfo
};
...
impl DataTransformService {
    ...
    async fn transform_pool_to_pool_info(
        ...
        // 获取mint A的元数据
        let mint_a = self.create_extended_mint_info(
            &pool.mint0.mint_address,
            pool.mint0.decimals,
            &pool.mint0.owner,
            metadata_map,
        )?;
        ...
        // 获取mint B的元数据
        let mint_b = self.create_extended_mint_info(
            &pool.mint1.mint_address,
            pool.mint1.decimals,
            &pool.mint1.owner,
            metadata_map,
        )?;
    ...
    fn create_mint_extensions(&self, mint_address: &str, metadata: Option<&TokenMetadata>) -> serde_json::Value {
        ...
        // 添加代币类型信息
        extensions.insert("type".to_string(), serde_json::Value::String(
            self.classify_token_type(mint_address, metadata)
        ));
        
        // 添加安全等级
        extensions.insert("security_level".to_string(), serde_json::Value::String(
            self.assess_security_level(mint_address, metadata)
        ));
        
        // 添加流动性等级估算
        extensions.insert("liquidity_tier".to_string(), serde_json::Value::String(
            self.estimate_liquidity_tier(mint_address)
        ));
...
mod tests {
    use super::*;
    use database::clmm_pool::model::{TokenInfo, PriceInfo, VaultInfo, ExtensionInfo, TransactionInfo, PoolStatus, SyncStatus, PoolType};
    ...
    async fn test_create_extended_mint_info() {
        ...
        let mint_info = transform_service.create_extended_mint_info(
