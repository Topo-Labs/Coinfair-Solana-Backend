...
mod tests {
    use super::*;
    use super::super::data_transform::{DataTransformService, AmmConfigCache};
    ...
    async fn test_three_tier_query_strategy() {
        ...
        let result1 = data_transform_service.get_amm_config_optimized(test_config_address).await;
        ...
        let start_time = Instant::now();
        let result2 = data_transform_service.get_amm_config_optimized(test_config_address).await;
    ...
    async fn test_batch_query_optimization() {
        ...
        match &batch_result {
            Ok(configs) => {
                ...
                for (address, config) in configs.iter() {
                    println!("配置 {}: tick_spacing={}, trade_fee_rate={}", 
        ...
        if batch_result.is_ok() && cache_result.is_ok() {
            // 缓存查询应该显著更快
            assert!(cache_query_time < batch_query_time / 2, 
                "缓存批量查询应该比首次查询快得多: 首次={:?}, 缓存={:?}", 
                batch_query_time, cache_query_time);
            println!("✅ 批量缓存优化生效，查询速度提升 {:.1}x", 
    ...
    async fn test_async_database_write() {
        ...
        // 3. 执行异步写入
        println!("🔄 执行异步数据库写入");
        data_transform_service.async_save_config_to_database(test_address, &test_config).await;
    ...
    async fn test_performance_monitoring() {
        ...
        // 2. 测试缓存操作性能
        let test_configs = (0..1000).map(|i| {
            (format!("test_address_{}", i), AmmConfigCache {
                protocol_fee_rate: 120000,
                trade_fee_rate: 500,
                tick_spacing: 10,
                fund_fee_rate: 40000,
        ...
        println!("  - 缓存命中率: {:.2}%", (found_count as f64 / test_configs.len() as f64) * 100.0);
    ...
    async fn create_mock_database() -> Result<database::Database> {
        ...
        database::Database::new(test_config).await.map_err(|e| anyhow::anyhow!("创建数据库失败: {}", e))
    }
    ...
    async fn test_optimization_benchmark() {
        ...
        if std::env::var("RUN_INTEGRATION_TESTS").is_ok() {
            ...
            // 4. 性能对比分析
            println!("📊 基准测试结果:");
            println!("  - 未优化版本耗时: {:?} (单独查询 {} 次)", unoptimized_time, test_addresses.len());
