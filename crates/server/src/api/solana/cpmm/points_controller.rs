...
pub fn points_routes() -> Router {
    Router::new()
        ...
        // 用户交易积分详情列表
        .route("/points/user-detail/:wallet_address", get(get_user_transaction_details))
}
...
///
/// # 响应
/// - 200: 查询成功，返回用户交易积分详情列表
/// - 400: 参数错误
/// - 500: 服务器错误
#[utoipa::path(
    get,
    path = "/api/v1/solana/events/cpmm/points/user-detail/{wallet_address}",
    params(
        ("wallet_address" = String, Path, description = "用户钱包地址"),
        ("page" = Option<u64>, Query, description = "页码，默认1"),
        ("page_size" = Option<u64>, Query, description = "每页数量，默认50，最大100")
    ...
    responses(
        (status = 200, description = "查询成功", body = TransactionDetailResponse),
        (status = 400, description = "参数错误", body = ApiResponse<ErrorResponse>),
        (status = 500, description = "查询失败", body = ApiResponse<ErrorResponse>)
    ),
    tag = "Points System"
...
pub async fn get_user_transaction_details(
    ...
    Path(wallet_address): Path<String>,
    Query(params): Query<HashMap<String, String>>,
) -> Result<Json<TransactionDetailResponse>, (StatusCode, Json<ApiResponse<ErrorResponse>>)> {
    info!("🔍 接收到用户交易积分详情查询请求: wallet_address={}", wallet_address);
    ...
    // 调用服务层
    match services.solana.get_user_transaction_details(&wallet_address, page, page_size).await {
        Ok(response) => {
            if response.success {
                if let Some(ref data) = response.data {
                    info!(
                        "✅ 用户交易积分详情查询成功: wallet_address={}, 返回{}条记录, 总计{}条",
                        wallet_address, data.point_list.len(), data.total
                    );
                } else {
                    info!("✅ 用户交易积分详情查询成功（无数据）: wallet_address={}", wallet_address);
                }
            } else {
                info!("⚠️ 用户交易积分详情查询返回错误: wallet_address={}", wallet_address);
            }
            Ok(Json(response))
        }
        Err(e) => {
            error!("❌ 用户交易积分详情查询失败: wallet_address={}, error={}", wallet_address, e);
            let error_response = ErrorResponse::new("TRANSACTION_DETAILS_QUERY_FAILED", &format!("查询用户交易积分详情失败: {}", e));
            Err((StatusCode::INTERNAL_SERVER_ERROR, Json(ApiResponse::error(error_response))))
        }
    }
}
