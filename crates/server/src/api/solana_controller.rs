...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "计算成功", body = RaydiumResponse<SwapComputeData>),
...
///     "outputMint": "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v",
///     "outputAmount": "100000000",
///     "otherAmountThreshold": "1102500000",
///     "slippageBps": 50,
///     "priceImpactPct": 0.1,
///     "referrerAmount": "0",
///     "routePlan": [...]
...
#[utoipa::path(
    ...
    request_body = TransactionSwapRequest,
    responses(
...
pub async fn transaction_swap_base_in(
    Extension(services): Extension<Services>,
    ValidationExtractor(request): ValidationExtractor<TransactionSwapRequest>,
    ...
    match services.solana.build_swap_transaction_base_in(request).await {
        Ok(transaction_data) => {
...
pub async fn transaction_swap_base_out(
    Extension(services): Extension<Services>,
    ValidationExtractor(request): ValidationExtractor<TransactionSwapRequest>,
) -> Result<Json<RaydiumResponse<Vec<TransactionData>>>, (StatusCode, Json<RaydiumErrorResponse>)> {
    info!("🔨 构建swap-base-out交易，钱包: {}", request.wallet);

    match services.solana.build_swap_transaction_base_out(request).await {
        Ok(transaction_data) => {
